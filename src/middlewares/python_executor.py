...
Compose atom tools sequential or parallel execution. To invoke atom tools within the `python_executor`, you should call `invoke`/`ainvoke` methods.
Each tool exposes `invoke`/`ainvoke` for synchronous and asynchronous calls, Arguments should be a dictionary passed as the first argument, for example:
```python
await some_tool.ainvoke({ 'query': 'Today news', 'type': 'news', 'timelimit': 'd' })
```
Define a `main` function to handle these tools' intermediate results and return the final result.
...
def _create_python_executor_tool(tools: Optional[dict[str, Callable]] = {}):
    ...
    return StructuredTool.from_function(
        ...
        description="""
            ...
            Return: The result value of the `main` function.
        """,
...
# Note: This is not normal stateless middleware, which stores agents' tools. It's best to create different instances of it.
